...
async function fetchWeatherInfo(lat, long, selectedLocation) {
    ...
    status.innerHTML = `Status: ${res.current.condition.text}`
    ...
    gust.innerHTML = `Wind gust: ${res.current.gust_kph} Km/h`
...
async function fetchSevenDayForecast(lat, long){
    ...
    await fetch(URL).then(res => res.json()).then(data => {
        const { daily } = data;
        res = daily;
        localStorage.setItem('sevenDayForecast', JSON.stringify(res));
    })
